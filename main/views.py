...
from .models import DuckHuntScore
...
def index(request):
    if request.method == 'POST':
        ...
        if player_name:
            return redirect(f'game/?player_name={player_name}') 
...
def save_score(request):
    if request.method == 'POST':
        try:
            ...
            DuckHuntScore.objects.create(player_name=player_name, score=score)
...
def scores_list(request):
    ...
    scores_list = DuckHuntScore.objects.order_by('-score')
    ...
    return render(request, 'main/scores_list.html', context)